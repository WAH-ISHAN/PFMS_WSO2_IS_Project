...
export default function Budget() {
  ...
  const add = async (e) => {
    ...
    try {
      setAdding(true);
      await api.post('/budgets', { category: category.trim(), amount: amt });
      setAmount('');
      await load();
    } catch {
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      setErr('Failed to delete budget');
    }
  };
    ...
    <div className="px-4 md:px-6">
      <div className="rounded-2xl border border-gray-200/60 dark:border-gray-700 bg-white/80 dark:bg-gray-900/60 backdrop-blur shadow-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Budgets</h2>
            <p className="text-sm text-gray-600 dark:text-gray-300">Plan and track your budgets.</p>
          </div>
          {/* You could add quick totals here if you want */}
        </div>
